...
from entities.TestingEntity import TestingEntity
...
nfont = pygame.font.Font("data/fonts/Eight-Bit Madness.ttf", 20)
pygame.display.set_caption('Ruins')
icon = pygame.image.load("data/images/rlogo.png")
icon.set_colorkey((0,0,0))
pygame.display.set_icon(icon)
WINDOW_SIZE = (1200, 800) # 600, 400 image size
...
status_effect_imgs = {"poison":pygame.image.load("data/images/status effects/poison.png"),"slow":pygame.image.load("data/images/status effects/slow.png")}
...
def draw_landscape(grass_sound_timer):
    ...
    true_scroll[0] += (PlayerHandshake.player.get_x() - true_scroll[0] - 152) / 20
    true_scroll[1] += (PlayerHandshake.player.get_y() - true_scroll[1] - 106) / 20
...
while True:  # game loop
    if Handshake.gamestate == 'dead':
        display.fill((0,0,0))
        boundingbox = pygame.draw.rect(display,(255,255,255),pygame.Rect(float(20),float(20),float(200),float(200)))
    if Handshake.gamestate == 'menu':
        display.fill((0, 0, 0))
        display.blit(pygame.image.load("data/images/rlogo.png"), (120, 30))
        display.blit(nfont.render("Press SPACEBAR to continue", False, (255, 255, 255)), (30, 120))
    if Handshake.gamestate == 'alive':
        ...
        maxhealthbar = pygame.draw.rect(display, (139, 0, 0),
                                        pygame.Rect((float(10), float(10)), (float(PlayerHandshake.player.max_health), float(10))))
        hpbar = pygame.draw.rect(display, (255, 127, 127),
                                 pygame.Rect((float(10), float(10)), (float(PlayerHandshake.player.health), float(10))))

        label = font.render(str(PlayerHandshake.player.health) + "/" + str(PlayerHandshake.player.max_health), False, (255, 255, 255))

        display.blit(label, (min(PlayerHandshake.player.max_health/2-10,190), 12))
        iterated = []
        hudix = 10
        hudiy = 30
        hudex = 10
        ...
        color = (255,255,255)
        goldc = (212,175,55)
        rainbow = get_score_color()
        if PlayerHandshake.player.score > 10:
            color = goldc
        if PlayerHandshake.player.score > 200:
            color = rainbow
        ...
        for event in pygame.event.get():  # event loop
            if event.type == KEYDOWN:
                if event.key == K_p:
                    particle_burst(100,(255,255,255),PlayerHandshake.player.entity.x,PlayerHandshake.player.entity.y)
                if event.key == K_h:
                    elist.append(HomingMissile(PlayerHandshake.player.entity.x, PlayerHandshake.player.entity.y-100))
                if event.key == K_i:
                    elist.append(Item(PlayerHandshake.player.entity.x, PlayerHandshake.player.entity.y, 'tester', 'Increases walk speed, jump strength and max hp',"potion of strength"))
                if event.key == K_e:
                ...
                if event.key == K_d:
                    PlayerHandshake.player.damage(random.randint(1, 10), 'physical_dmg',None)
                if event.key == K_f:
                    PlayerHandshake.player.heal(random.randint(1, 10), 'magical_heal')
                if event.key == K_a:
                    PlayerHandshake.player.attack()
                if event.key == K_t:
                    elist.append(TestingEntity(PlayerHandshake.player.entity.x,PlayerHandshake.player.entity.y-100))
                if event.key == K_z:
